...
function SidebarEntry({ item, active, onClick }: { item: MenuItem; active: boolean; onClick: () => void }) {
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        "group relative flex h-10 w-full items-center gap-2.5 rounded-control px-3 text-left text-sm transition-colors",
        active
          ? "bg-[#ecfdf5] font-bold text-vetvax-primary"
          : "bg-transparent font-semibold text-[#334155] hover:bg-[#f1f5f9] hover:text-vetvax-text-main",
      )}
    >
      <span className={cn("absolute inset-y-1 left-0 w-[3px] rounded-pill bg-vetvax-primary", active ? "opacity-100" : "opacity-0")} />
      <item.icon className={cn("h-4 w-4", active ? "text-vetvax-primary" : "text-vetvax-text-tertiary group-hover:text-vetvax-primary")} />
...
function SidebarGroup({
  ...
  return (
    <div className="mt-7 first:mt-0">
      <p className="mb-2 px-1 text-[11px] font-bold uppercase tracking-[0.08em] text-[#94a3b8]">{title}</p>
...
export default function Sidebar({ activeKey, onNavigateDone }: { activeKey: NavKey; onNavigateDone?: () => void }) {
  ...
  return (
    <aside className="flex h-full min-h-screen w-[248px] flex-col border-r border-vetvax-border-soft bg-white px-4 py-5">
      <div className="flex items-center gap-3">
        <div className="grid h-9 w-9 place-items-center rounded-[12px] bg-vetvax-primary text-sm font-extrabold text-white">V</div>
        <div>
      ...
      <div className="mt-[18px] h-px bg-vetvax-border-soft" />
      ...
      <div className="mt-4 flex h-14 items-center justify-between rounded-[16px] border border-vetvax-border-soft bg-[#f8fafc] px-3">
